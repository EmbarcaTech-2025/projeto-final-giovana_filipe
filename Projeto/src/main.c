...
char read_keypad()
{
    ...
    if (current_time - last_key_time < 250)
    { // Aumentado para 250ms para melhor debounce
        return '\0';
    }
            ...
            if (gpio_get(gpio_cols[col]) == 0)
            {
                ...
                { // Confirma a leitura
                    ...
                    if (gpio_get(gpio_cols[col]) == 0)
                    {
                        ...
                        int timeout = 0;
                        while (gpio_get(gpio_cols[col]) == 0 && timeout < 200)
                        {                // Aumentado timeout
...
int main()
{
    ...
    while (1)
    {
        ...
        case MENU_TRAVAR_DESTRAVAR:
        {
            ...
            { // Caixa fechada (iluminação igual a zero)
