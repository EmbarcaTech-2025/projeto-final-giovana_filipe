...
// Função simplificada para inicializar BMP280
void bmp280_init(i2c_inst_t* i2c) {
    bmp280_i2c = i2c;
    
    // Reset BMP280
    uint8_t reset_cmd[] = {0xE0, 0xB6};
    i2c_write_blocking(bmp280_i2c, 0x76, reset_cmd, 2, false);
    sleep_ms(100);
    
    // Configure BMP280 for normal mode
    uint8_t config[] = {0xF4, 0x27}; // Normal mode, 1x oversampling
    i2c_write_blocking(bmp280_i2c, 0x76, config, 2, false);
    sleep_ms(10);
}

// Função simplificada para ler temperatura e umidade
void bmp280_read_temp_humidity(float* temperature, float* humidity) {
    if (bmp280_i2c == NULL) {
        *temperature = 25.0f;
        *humidity = 50.0f;
        return;
    ...
    i2c_write_blocking(bmp280_i2c, 0x76, &temp_reg, 1, true);
    i2c_read_blocking(bmp280_i2c, 0x76, temp_data, 3, false);
    
    int32_t temp_raw = (temp_data[0] << 12) | (temp_data[1] << 4) | (temp_data[2] >> 4);
    
    // Simple temperature calculation (simplified)
    *temperature = (temp_raw / 5120.0f) + 0.5f;
    
    // For humidity, we'll use a simple approximation based on temperature
    // In a real implementation, you'd use a separate humidity sensor
    *humidity = 50.0f + (25.0f - *temperature) * 2.0f;
